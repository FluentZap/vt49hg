...
diceResult DiceRoller::diceRoll(byte statistic, byte skill, byte difficulty, byte difficultyUpgrade, byte blue, byte black, byte white) {
	...
	while(rolledDice < diceToRoll) {
		if(green > 0) {
			rolledResult[rolledDice] = rollGreen();
			rolledDice++;
			green--;
		}
		if(yellow > 0) {
			rolledResult[rolledDice] = rollYellow();
			rolledDice++;
			yellow--;
		}
		if(purple > 0) {
			rolledResult[rolledDice] = rollPurple();
			rolledDice++;
			purple--;
		}
		if(red > 0) {
			rolledResult[rolledDice] = rollRed();
			rolledDice++;
			red--;
		}
		if(blue > 0) {
			rolledResult[rolledDice] = rollBlue();
			rolledDice++;
			blue--;
		}
		if(black > 0) {
			rolledResult[rolledDice] = rollBlack();
			rolledDice++;
			black--;
		}
		if(white > 0) {
			rolledResult[rolledDice] = rollWhite();
			rolledDice++;
			white--;
		}
	}
